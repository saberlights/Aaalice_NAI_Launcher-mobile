...
class _Img2ImgPanelState extends ConsumerState<Img2ImgPanel> {
  ...
  Widget _buildSourceImageSection(
    ...
    // 1. 构建底层内容（根据是否有图，显示占位框或真实图片）
    if (!hasSourceImage) {
      content = _buildEmptySourceSection(theme);
    } else {
    ...
    // 🌟 2. 核心：加上 Stack 遮罩层。如果正在导入，直接盖上黑色半透明遮罩和转圈动画！
    return Stack(
      children: [
        content,
  ...
  (int, int) _resolveSourceDimensions(
    ...
    try {
      ...
      // 👈 适配旧版本 image 库的写法，改用 startDecode
      // 它同样只会读取图片头部的尺寸信息，不会暴力解码全图，完美解决卡死问题
      final info = decoder?.startDecode(sourceBytes);
      if (info != null && info.width > 0 && info.height > 0) {
        return (info.width, info.height);
      }
    } catch (e) {
      AppLogger.w('Failed to decode image info: $e', 'Img2ImgPanel');
    }
    
    // 如果极端情况下解析失败，返回一个合理的默认值，绝不阻塞主线程
    return (1024, 1024);
  }
            ...
            if (!comfyEnabled)
              ...
              if (!isComfyRtx) ...[
                ...
                DropdownButtonFormField<String>(
                  ...
                  isExpanded: true, // 👈 【核心修复】加上这行，强制文字截断！
