...
class _TagLibraryPageScreenState extends ConsumerState<TagLibraryPageScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return PageShortcuts(
      ...
      shortcuts: shortcuts,
      child: Scaffold(
        // 【修复】：在手机端，把左侧分类树变成可以滑出的“抽屉(Drawer)”
        // 这样就能把 100% 的屏幕宽度还给右侧的词条列表！
        drawer: Drawer(
          child: SafeArea(
            child: _buildCategorySidebar(theme, state),
          ),
        ),
        body: Column(
          children: [
            // 顶部工具栏（集成批量操作）
            // 我们给它套上 SafeArea，防止被刘海遮挡
            SafeArea(
              bottom: false,
              child: TagLibraryToolbar(
...
// ... 保持下面的代码不变 ...
  ...
  Future<void> _handleSendToFixedTag(
    ...
    await ref.read(fixedTagsNotifierProvider.notifier).addEntry(
          ...
          categoryId: entry.categoryId, // 👈 加上这行，原作者新增了分类ID的同步
        );
    if (!mounted) return;
