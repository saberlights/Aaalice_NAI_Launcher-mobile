...
class _WeightAdjustToolbarState extends State<_WeightAdjustToolbar> {
  ...
  Offset _calculateOffset() {
    ...
    const verticalPadding = 40.0; // 给系统文本菜单让出的高度
    const toolbarWidth = 220.0;   // 🌟 工具栏的固定宽度

    // 🌟 核心修复：X轴智能边缘保护
    double dx = caretRect.left;
    // 计算允许的最大 X 坐标（输入框总宽度 - 工具栏宽度）
    final maxDx = textFieldRenderBox.size.width - toolbarWidth;
    
    // 如果光标太靠右，工具栏会越界，强制把它往左拉回安全区域！
    if (dx > maxDx) {
      dx = maxDx;
    }
    // 如果光标太靠左（理论上很少见，保险起见），强制对齐最左侧！
    if (dx < 0) {
      dx = 0;
    }
