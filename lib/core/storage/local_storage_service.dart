...
class LocalStorageService {
  ...
  /// 获取是否启用多人提示词智能拆分 (初始默认关闭)
  bool getMultiCharParseEnabled() {
    // 👈 这里的 defaultValue: false 就是核心，第一次安装时默认给 false
    return getSetting<bool>(
          'multiCharParseEnabled', 
          defaultValue: false,
        ) ??
        false;
  }
  ...
  /// 保存是否启用多人提示词智能拆分
  Future<void> setMultiCharParseEnabled(bool value) async {
    await setSetting('multiCharParseEnabled', value);
  }
